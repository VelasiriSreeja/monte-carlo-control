...
## MONTE CARLO CONTROL ALGORITHM
tep 1 Initialize the action-value function 𝑄(𝑠,𝑎)arbitrarily for all state-action pairs, and initialize a policy 𝜋 based on 𝑄(𝑠,𝑎).

step 2 Generate an episode using the current policy π, which consists of a sequence of state-action-reward tuples

step 3 Compute the return 𝐺𝑡(cumulative reward from time 𝑡).

step 4 Update 𝑄(𝑠𝑡,𝑎𝑡) by averaging the observed returns.

step 5 Improve the policy 𝜋(𝑠)=argmax 𝑄(𝑠,𝑎). The Monte Carlo control function updates the action-value function Q by averaging returns. It can be expressed as: image image where α is a learning rate, and G is the return (cumulative reward). This function helps in refining Q(s,a) values, which subsequently improves the policy for maximizing the expected reward.
...
![Screenshot 2024-10-22 105843](https://github.com/user-attachments/assets/3fd4fe31-95ca-442f-bb73-fccdf639c87c)

![Screenshot 2024-10-22 105859](https://github.com/user-attachments/assets/2c8be1cd-4813-4eb3-a0d0-12a1fa19652c)
